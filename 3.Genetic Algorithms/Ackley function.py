...
def decode_chromosome2(chromosome2):
    global L_chromosome, N_chains,a,b
    value = 0 
    for p in range(int(L_chromosome/2), L_chromosome):
        value+=(2**(p-(1+int(L_chromosome/2))))*chromosome2[-1-(p-1-int(L_chromosome/2))]
    return a+(b-a)*float(value)/(N_chains-1)
def f(x,y):
    #return .5*+math.pow(x,2)-10*math.cos(2*math.pi*x)
    a = 20
    b = -0.2
    c = 2*math.pi
...
def evaluate_chromosomes():
    ...
    for p in range(N_chromosomes):
        ...
        fitness_values[p]=f(v,m)
        
def compare_chromosomes(chromosome1,chromosome2):
    vc1 = decode_chromosome(chromosome1)
    vc3 = decode_chromosome2(chromosome1)
    print(vc1,vc3)
    vc2 = decode_chromosome(chromosome2)
    vc4 = decode_chromosome2(chromosome2)
    fvc1=f(vc1,vc3)
    fvc2=f(vc2,vc4)
    if fvc1 > fvc2:
        return 1
...
def nextgeneration():
    ...
    print( "f("+str(decode_chromosome(F0[0]))+","+str(decode_chromosome2(F0[0]))+")= "+
           str(f(decode_chromosome(F0[0]),decode_chromosome2(F0[0])))) 
